...
test.describe('Dashboard Review Flow', () => {
  test('shows chores awaiting review on the dashboard', async ({ page }) => {
  ...
  test('displays correct reward for split chore (non-joined)', async ({
    page,
  }) => {
    await loginAsParent(page)
  ...
  test('displays correct reward for joined chore (pooled)', async ({
    page,
  }) => {
    await loginAsParent(page)
  ...
  test('shows Rate All button for multi-kid chores', async ({ page }) => {
    ...
    await expect(
      page.getByRole('button', { name: /Rate All/i }).first(),
    ).toBeVisible()
  })

  test('Rate All dialog shows correct calculations for joined chore', async ({
    page,
  }) => {
    await loginAsParent(page)

    // Find the joined chore card (Umýt nádobí) by locating the heading and navigating to its Card container
    const joinedChoreHeading = page.getByText('Umýt nádobí')
    // Get the Card container (parent elements up to the card boundary)
    const joinedChoreCard = page
      .locator('[class*="rounded-xl border"]')
      .filter({ has: joinedChoreHeading })
    ...
    await expect(
      page.locator('[role="dialog"]').getByText('20.00 Kč'),
    ).toBeVisible()

    // Both Alice and Bob should be listed
    await expect(
      page.locator('[role="dialog"]').getByText('Alice'),
    ).toBeVisible()
    await expect(page.locator('[role="dialog"]').getByText('Bob')).toBeVisible()
  })

  test('quality coefficient affects reward display - buttons show bad/good/excellent amounts', async ({
    page,
  }) => {
    await loginAsParent(page)
